...
int GXParseJSON ( char* text, size_t len, size_t count, JSONValue_t* where )
{
        ...
        for(; i < len; i++)
        {
            ...
            else if (text[i] == '{')                                                               // Any time we hit a '{', we have come across an object, and need to skip over it. 
            {
                ...
                {	
                    ...
                    // FIX: Randomly doesn't work... What the fuck.					
        ...
        if (*text == '\"')
        {
            ...
            *text = 0;                                                                             // Set a null terminator at the end of the string, overwriting the '\"' with a '\0'.
            ...
            else if (*text == '{')                                                                 // Parse out an object
            {
                ...
                where[currentWhere].type           = GXJSONobject;                                 // Set the type as an object
            ...
            else if ((*text >= '0' && *text <= '9') || *text == '.' || *text == '-')              
            {
                ...
                where[currentWhere].type           = GXJSONprimative;                             // Set the type as a primative    
    ...
    exitNotAllParsed:
            ...
            printf("Not all JSON tokens were parsed. Only %ld out of %ld were parsed", currentWhere, count);
